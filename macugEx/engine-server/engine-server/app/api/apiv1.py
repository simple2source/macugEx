...
def api_group_modify_user_info(data):
    ...
    if user_id in group.get('users', tuple()):
...
def api_watch_get_authcode(data):
    watch = db.watch.find_one({'_id': data['imei']}, {'_id': 1})
    if not watch:
...
def api_group_new_watch(data):
    ...
    if not watch:
        return failed(E_watch_nofind)
    if 'group_id' in watch:
        if watch['group_id'] == data['group_id']:
...
def api_group_join(data):
    ...
    if data['group_password'] and data['group_password'] != group['password'] and not data['invitecode']:
        return failed(E_group_password)
...
def api_group_del_contact(data):
    ...
    if 'contacts' in group:
        if data['phone'] not in group['contacts']:
            return failed(E_group_nocontact)
        if not group['contacts'][data['phone']].get('status', 0):
            return failed(E_group_nocontact)
    else:
        return failed(E_group_nocontact)
    now = time.time()
...
def api_watch_unlock(data):
    ...
    if not watch:
        return failed(E_watch_nofind)
    if watch.get('status') == 1:
        return succed()
    result = agent.send(data['imei'], '\x14', '')
    if result == OK:
        db.watch.update_one({'_id': data['imei']}, {'$set': {'status': 1}})
        return succed()
    elif result == NO:
        return failed(E_watch_response_offline)
    else:
...
def api_watch_monitor(data):
    ...
    if user_id not in group.get('users', tuple()):
        return failed(E_user_notin_group)
    phone = group['users'][user_id].get('phone')
    if not phone:
...
def api_watch_alarm_set(data):
    ...
    if data['id']:
        # 删除或修改闹钟
        if 'alarms' not in watch or data['id'] not in watch['alarms']:
            return failed(E_watch_alarm_nofind)
        if data['status'] == 'delete':
            db.watch.update_one({'_id': data['imei']}, {'$unset': {'alarms.%s' % data['id']: 1}})
            del watch['alarms'][data['id']]
            alarm = watch['alarms'].copy()
...
def api_watch_alarm_get(data):
    ...
    alarm_list = []
    if 'alarms' in watch:
        for alarm_id, alarm in watch['alarms'].items():
            alarm['id'] = alarm_id
...
def api_watch_info(data):
    ...
    if not group:
        return failed(E_group_nofind)
    if 'devs' in group:
        if data['imei'] not in group['devs'] or not group['devs'][data['imei']].get('status'):
            return failed(E_watch_notin_group)
    else:
        return failed(E_watch_notin_group)
...
def api_face_get_session(data):
    ...
    if candidate['confidence'] < 80:
        ...
        if 'faces' not in condidate_user:
            face_person_delete(person_id=candidate['person_id'])
            return failed(E_face_nothas_user)
        for face in condidate_user['faces'].values():
...
def api_plaza_delete(data):
    ...
    if not plaza:
        return failed(E_plaza_notfind)
    if plaza['user_id'] != data['session_user_id']:
