...
user_channel = '%s/user/' % conf.mqtt['prefix'] + '%s'
group_channel = '%s/group/' % conf.mqtt['prefix'] + '%s'
...
logfile = os.path.abspath(os.path.join(os.path.dirname(conf.__file__), conf.push['logfile']))
...
File_logging.setLevel(conf.push['loglevel'])
logger.addHandler(File_logging)
logger.setLevel(conf.push['loglevel'])

if __name__ == '__main__':
    sys.excepthook = sys_exc_hook
    get_hub().print_exception = gevent_exc_hook
    host = '0.0.0.0' if conf.push['host'] != '127.0.0.1' else '127.0.0.1'
    server = StreamServer(('127.0.0.1', conf.push['port']), sock_handle, backlog=100000)
