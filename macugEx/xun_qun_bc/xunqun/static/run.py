...
static = Flask('static')
use_nginx = setting.static['use_nginx']

# 头像模式中, normal级别,small级别的大小
normal_size = setting.static['normal_size']  # app端查看头像大小
...
# file: 所下载星历文件, 'timestamp': 星历文件有效时间,星历更新周期最好小于其有效时间
for alp in [
    {'file': 'current_1d.alp', 'timeout': 3600 * 24 * 1},
    {'file': 'current_2d.alp', 'timeout': 3600 * 24 * 2},
    {'file': 'current_3d.alp', 'timeout': 3600 * 24 * 3},
    {'file': 'current_5d.alp', 'timeout': 3600 * 24 * 5},
    {'file': 'current_7d.alp', 'timeout': 3600 * 24 * 7},
    {'file': 'current_10d.alp', 'timeout': 3600 * 24 * 10},
    {'file': 'current_14d.alp', 'timeout': 3600 * 24 * 14},
]:
    if setting.server['almanac_timeout'] <= alp['timeout']:
        almanac_timeout = alp['timeout']
        almanac_file = alp['file']
        break
else:
    raise ValueError('setting almanac timeout too long')
...
def download_almanac():
    try:
        for i in range(5):
            try:
                web_content = urllib.urlopen('https://alp.u-blox.com').read()
                break
            except:
                gevent.sleep(10)
                if i == 4:
                    raise
        search = re.search('\d{10}', web_content)
        if search:
            update_timestamp = int(search.group())
            expire_timestamp = int(update_timestamp + almanac_timeout)
            redis.set('AlmanacTimestamp', expire_timestamp)
        else:
            raise ValueError('not timestamp find in u-box web')
        content = urllib.urlopen('https://alp.u-blox.com/%s' % almanac_file).read()
        if content:
            with open(alp_filename, 'wb') as f:
                f.write(content)
            global alp_file_length
            alp_file_length = len(content)
        else:
            logger.error('download almanac fail')
    except:
        logger.error('in download almanac', exc_info=True)


def check_almanac():
    while 1:
        gevent.sleep(almanac_timeout / 2)
        download_almanac()


logger = logging.getLogger('static')
logfile = setting.static['logfile']
...
File_logging.setLevel(setting.static['loglevel'])
logger.addHandler(File_logging)
logger.setLevel(setting.static['loglevel'])
...
if __name__ == '__main__':
    ...
    if os.path.exists(alp_filename):
        # 星历文件更新时间在一天前时更新星历文件
        if time.time() - os.stat(alp_filename).st_ctime > (almanac_timeout / 2):
            gevent.spawn(download_almanac)
    else:
        gevent.spawn(download_almanac)
    gevent.spawn(check_almanac)

    sys.excepthook = sys_exc_hook
    get_hub().print_exception = gevent_exc_hook
    if setting.static['use_local']:
        ...
        server = WSGIServer(('', setting.static['use_port']), static, log=None, backlog=1024)
