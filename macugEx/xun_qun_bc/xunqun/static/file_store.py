...
small_size = setting.static['small_size']  # 腕表端查看头像大小


def image_drain_thumbnail(data, image_path_prefix, image_file_id, normal_size=normal_size, small_size=small_size):
    buff = io.BytesIO()
    buff.write(data)
    buff.seek(0)
...
def story_image_put(data):
    oid = ObjectId()
    # 故事所用的图片为 500x500, 只在APP上显示, 手表上显示的为故事文件内的封面图片, 索引图片等
    image_drain_thumbnail(data, '../cache/story/', oid, (500, 500), None)
    return oid
