...
def pusher():
    try:
        client = mqtt.Client(client_id='%s_%s' % (setting.mqtt['prefix'], str(random.randint(0, 999999999))))
        client.username_pw_set(setting.mqtt['username'], setting.mqtt['password'])
        client.connect(setting.mqtt['host'], setting.mqtt['port'])
...
def recver():
    try:
        ...
        client.connect(setting.mqtt['host'], setting.mqtt['port'])
        client.subscribe('%s/app/status' % setting.mqtt['prefix'])
        loop = gevent.get_hub().loop
        watch1 = loop.io(client.socket().fileno(), READ)
        watch1.start(handle_read, client)
        watch2 = loop.io(client._sockpairR.fileno(), READ)
