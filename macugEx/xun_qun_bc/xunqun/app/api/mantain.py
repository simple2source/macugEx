...
from static.define import banner_image_path, category_image_path, story_audio_path, story_image_normal_path, \
...
if not db.appstore.category.find_one():
...
def get_medal_question_list(medal_id):
    return medal_question_dict.get(medal_id, [])


def fresh_medal_list():
    if not medal_list:
        for medal in db.medal.find({}).sort('sort', 1):
            goals = []
            for story in db.story.find({'_id': {'$in': [ObjectId(story_id) for story_id in medal['story_list']]}}, {
                'title': 1,
            }):
                goals.append({
                    'story_id': str(story['_id']),
            ...
            question_list = []
            for Q in db.answer_game.find({'_id': {'$in': [ObjectId(qid) for qid in medal['question_list']]}}):
                Q['_id'] = str(Q['_id'])
                question_list.append(Q)
            question_list.sort(lambda x, y: medal.index(str(x['_id'])) - medal.index(str(y['_id'])))
            medal_question_dict[medal['_id']] = question_list
