...
from core.db import db, redis, put_dev_job, message_audio, message_image, user_face
...
def add_user_groups(user_id, group_id, now, old_user_groups=None):
    ...
    with redis.pipeline() as pipe:
        for i in range(5):
            pipe.watch('User:%s' % user_id)
            groups, devicetoken = pipe.hmget('User:%s' % user_id, 'groups', 'app_token')
            if groups:
            ...
            try:
                pipe.multi()
                pipe.hset('User:%s' % user_id, 'groups', new_groups)
                pipe.execute()
...
def del_user_groups(user_id, group_id, now, old_user_groups=None):
    ...
    with redis.pipeline() as pipe:
        for i in range(5):
            group_id = str(group_id)
            pipe.watch('User:%s' % user_id)
            groups = pipe.hget('User:%s' % user_id, 'groups')
            if groups:
                groups = json.loads(groups)
                if isinstance(old_user_groups, dict):
                    for i, j in groups.items():
            ...
            try:
                pipe.multi()
                pipe.hset('User:%s' % user_id, 'groups', json.dumps(groups))
                pipe.execute()
...
def new_group_mail(email, group_id, group_password):
    ...
    send_mail(email, '创建家庭圈-Kwatch', group_html % (
        group_id,
...
def send_mail(recver, topic, content):
    try:
        msg = MIMEMultipart()
        msg['From'] = 'KWatch团队<admin@kwatch.bluetooth.la>'
        msg['To'] = recver
        msg['Subject'] = topic
        ...
        smtp.connect('cloud.submail.cn', 25)
        smtp.ehlo()
        # smtp.starttls()
        smtp.login('11175', 'f77c4221800f43744e0959a2ad08c372')
        smtp.sendmail(
            'admin@kwatch.bluetooth.la',
            recver,
            data
        )
    except:
...
face_key = 'ad2635a75277f1bf381fe0ad76d847cb'
face_secret = 'nEKhWD83uW15pAoUo70YiP4mNfDf4Eo4'
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), './config.json'), 'r') as f:
    customer_config = json.loads(f.read())


def verify_identify_customer(identity, customer_id):
    """
    检测APP包名与腕表客户号权限

    usage:
        errno = verify_identify_customer(identity, customer_id)
        if errno:
            return failed(errno)
    """
    try:
        if customer_config[identity] == customer_id:
            return 0
        else:
            return E_different_agent
    except KeyError:
        return 0
