...
def question_new(data):
    now = time()
    qid = str(db.question.insert_one({
...
def question_serve(question_id, data):
    ...
    if status == 1:
        now = time()
        mid = str(db.question_message.insert_one({
...
def question_message_send_text(question_id, data):
    ...
    if data['sender_type'] != 1 and status == 1:
        # 客服不能回答未开始的问题
        return respond({'status': 300, 'error': E_quest_unstart, 'debug': 'question not yet started'})
    ...
    if data['sender_type'] == 1:
        ...
        if status == 2:
            ...
            push_user = None
    elif data['sender'] != question['serv_id']:
        return respond({'status': 300, 'error': E_quest_bad_serv_id, 'debug': 'service id does not match'})
    else:
        push_user = question['user_id']

    if status == 3:
        db.question.update_one({'_id': q_id}, {'$set': {'status': 2}})
...
def question_message_send_image(question_id, data):
    try:
        ...
        return respond({'status': 300, 'error': E_params, 'field': 'question_id', 'debug': 'unregular ObjectId'})
    question = db.question.find_one({'_id': q_id}, {'user_id': 1, 'serv_id': 1, 'status': 1})
    if not question:
        return respond({'status': 300, 'error': E_quest_nofind})
    status = question.get('status')
...
@app.route('/question/<question_id>/reopen', methods=['POST'])
@validate_decorator(form.Session())
def reopen(question_id, data):
    try:
        q_id = ObjectId(question_id)
    except InvalidId:
        return respond({'status': 300, 'error': E_params, 'field': 'question_id', 'debug': 'unregular ObjectId'})
    ...
    status = question.get('status')
    if status == 3:
        if data['session_user_id'] != question['user_id']:
            return respond({'status': 300, 'error': E_quest_bad_user_id, 'debug': 'user id does not match'})
        now = time()
        mid = str(db.question_message.insert_one({
            'question_id': question_id,
            'type': 4,
